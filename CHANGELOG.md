...
- Stable release focused on regular `.html` files using TextMate injection grammar.
- Added extension icon support via `icon` manifest field.
- Removed redundant `activationEvents` now auto-generated by VS Code from contributions.
- Consolidated formatter and script-type transforms for `jsx`, `text/jsx`, and `text/babel`.
- Removed obsolete `html-jsx` language mode scope and legacy workspace association flow.
- Kept production safeguards: signed conventional commits, CI checks, and unit tests.
